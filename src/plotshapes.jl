function make_rect_patches(ints::AbstractVector{<:NTuple{2,<:Number}}, ycenter, height = 1)
    half_height = height / 2
    y_bottom = ycenter - half_height
    n_int = length(ints)
    rects = Vector{Py}(undef, n_int)
    for (i, (xb, xe)) in enumerate(ints)
        rects[i] = PythonPlot.matplotlib.patches.Rectangle((xb, y_bottom), xe - xb, height)
    end
    rects
end
...
function circle_collection(xs, ys, rad)
    ...
    patches = Vector{Py}(undef, nx)
    for i = 1:nx
        patches[i] = PythonPlot.matplotlib.patches.Circle((xs[i], ys[i]), rad)
    end
    patches
end
...
function rect_collection(xs, ys, dx, dy)
    ...
    for i = 1:nx
        patches[i] =
            PythonPlot.matplotlib.patches.Rectangle((xs[i] - x_off, ys[i] - y_off), dx, dy)
    end
    patches
end
...
function electrode_grid(assembly_type::Symbol; kwargs...)
    ...
    elseif assembly_type == :HARBI
        patches = circle_collection(HARBI_XS, HARBI_YS, HARBI_PITCH / 2)
